...
class SourceScanTests(unittest.TestCase):
    def test_unimported_library_module_fails(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "Equipartition").mkdir()
            (root / "Equipartition" / "Hidden.lean").write_text(
                "theorem checked : True := True.intro\n")
            (root / "Equipartition.lean").write_text("-- import Equipartition.Hidden\n")
            self.assertEqual(check_proofs.check_library_imports(root),
                             ["Library module missing from aggregate: Equipartition.Hidden"])
            (root / "Equipartition.lean").write_text("import Equipartition.Hidden\n")
            self.assertEqual(check_proofs.check_library_imports(root), [])
...
class LeanAuditTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.audit_commands = source.split("open Lean Elab Command\n", 1)[1].split(
            "\naudit_axioms Equipartition.", 1
        )[0]

    def run_fixture(self, source: str) -> subprocess.CompletedProcess[str]:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            fixture = root / "Equipartition" / "AuditFixture.lean"
            ...
            audit.write_text(
                "import Equipartition.AuditFixture\n"
